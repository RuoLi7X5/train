...
- **技术点**：
  ...
  - 规则引擎：落子合法性校验、提子计算、劫争位置记录与禁入判断。

### 4.4 实施路径 (Phase 4 Delivery Plan)
- **落地步骤**：
  - 设计题目数据结构与数据库迁移（棋谱数据、先手方、落子模式、文字描述）。
  - 开发出题棋盘编辑器（全棋盘视图、落子模式切换、先手与描述确认、试下模式入口）。
  - 实现规则引擎与落子校验（气、提子、打劫），同时用于学生端与出题端试下。
  - 完成智能裁剪渲染与推送展示（1/4、1/2、全棋盘兜底）。
  - 对接学生端盲棋交互与答案比对流程。
