...
## 阶段一：架构重构与多租户体系 (Architecture & Multi-Tenancy)
**目标**：打好地基，支持多教练模式，优化基础用户体验。这是后续所有功能的基础。

### 1.1 多教练/多租户系统 (Feature 5)
- **需求**：
  - **超级管理员 (Super Admin)**：
    - 账号：`ruoli` (ID: `R000000`)。
    - 权限：拥有最高权限，唯一功能是管理（创建/禁用）教练账号。
    - 个人教练账号需由 `ruoli` 另行创建。
  - **教练 (Coach)**：
    - 由超级管理员创建。
    - 管理名下的学生。
...
- **技术点**：
  ...
  - **状态机**：User 表增加 `status` (PENDING, ACTIVE)。
  - **ID 生成算法**：基于当前最大 ID 递增，支持回滚删除后的 ID 复用。

### 1.3 状态管理与体验优化 (Feature 2)
- **需求**：
...
### 1.3 学生端信息完善 (Feature 4)
- **需求**：
  - 学生首页昵称旁显示班级信息。
  - 预留积分排名与成就荣誉的 UI 区域。
- **技术点**：
  - 修改 `dashboard` 接口，联查 `Class` 信息。
